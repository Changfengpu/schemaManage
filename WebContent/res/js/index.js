...
var vm = new Vue({
    ...
    data: {
        userInfo:"",
        isPreTask:'',
        userSignNumber:'',//当天标注的条目量
        userSignSentence:'',//预留的显示当天标注的句子量，目前不需要展示
        userTaskSignNumber:'',
        target_score:'',
        result: [],   // list
        resp: [],     // submit
        ...
        tempName(newval, oldval) {
            ...
            for (var i = 0, j = template.length; i < j; i++) {
                if (template[i].relationName == newval) {
                    ...
                    this.explain=template[i].explain;
    ...
    methods: {
        getList_Ajax() {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    _this.getUserSignNumber()
                    //_this.getUserSignSentence()

                },
                error: function () {
                    _this._tip("error", "报错了", "可能原因是数据库已经没有数据可以标注了");
                }
            });
        ...
        submit_Ajax() {
            ...
            }else if(_this.isPreTask==1){
                $.ajax({
                    url: "preSignResultIntoMySql.do",
                    data: JSON.stringify(_this.resp),
                    dataType: "json",
                    contentType: "application/json",
                    cache: true,
...
//            })
        ...
        _getCheckboxData(event, domData, index) {
            if(this.result[index].choose.length == 0){return}
        ...
        getUserSignNumber() {
            let that = this;
            $.ajax({
                url: "findUserSignNumberInProjectByDay.do",
                data: {
       ...
       /* //当天标记的句子数量
        getUserSignSentence() {
            let that = this;
            $.ajax({
                url: "findUserSignSentenceInProjectByDay.do",
                data: {
                    tableName: that.tableName,
        ...
        indexCheck(){
            var that=this;
                that.getTemplateList_Ajax();
                that.target_score = getUrlParam('target_score')
                let id = getUrlParam('id')
                that.marker = that.userInfo.userName
                $.ajax({
                    url: "findUserTaskNumber.do",
                    data: {
                        user_id: that.userInfo.id,
                        project_id: id
                    },
